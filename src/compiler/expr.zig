...
pub fn compileExpr(self: *Compiler, e: *const Expr) !void {
    switch (e.*) {
        ...
        .BoolLiteral => |v| {
            if (v.val) {
                _ = self.emit(OpCode.LoadTrue, .None);
            } else {
                _ = self.emit(OpCode.LoadFalse, .None);
            }
        ...
        .BinaryOp => |bin| {
            ...
            const oc: OpCode = if (self.binaryOpSpecialized(bin.op, left_type, right_type)) |oc_spec|
                oc_spec
            else if (std.mem.eql(u8, bin.op, "+")) .BinaryAdd else if (std.mem.eql(u8, bin.op, "-")) .BinarySub else if (std.mem.eql(u8, bin.op, "*")) .BinaryMul else if (std.mem.eql(u8, bin.op, "/")) .BinaryDiv else if (std.mem.eql(u8, bin.op, "%")) .BinaryMod else if (std.mem.eql(u8, bin.op, "^")) .BinaryPow else if (std.mem.eql(u8, bin.op, "==")) .BinaryEq else if (std.mem.eql(u8, bin.op, "!=")) .BinaryNe else if (std.mem.eql(u8, bin.op, "<")) .BinaryLt else if (std.mem.eql(u8, bin.op, ">")) .BinaryGt else if (std.mem.eql(u8, bin.op, "<=")) .BinaryLe else if (std.mem.eql(u8, bin.op, ">=")) .BinaryGe else if (std.mem.eql(u8, bin.op, "&&") or std.mem.eql(u8, bin.op, "and")) .BinaryAnd else if (std.mem.eql(u8, bin.op, "||") or std.mem.eql(u8, bin.op, "or")) .BinaryOr else return error.UnknownBinaryOp;
            _ = self.emit(oc, .None);
        ...
        .CallExpr => |call| {
            ...
            if (call.callee.* == .Identifier) {
                    ...
                    std.mem.eql(u8, name, "file_exists"))
                {
                    ...
                    const builtin_op: OpCode = if (std.mem.eql(u8, name, "os_system")) .System else if (std.mem.eql(u8, name, "file_read")) .FileRead else if (std.mem.eql(u8, name, "file_write")) .FileWrite else .FileExists;
                    _ = self.emit(builtin_op, .None);
                    return;
                }
            }
            ...
            if (call.callee.* == .PropertyAccess) {
                ...
                _ = self.emit(OpCode.LoadConst, BytecodeArg{ .Int = @as(i32, @intCast(idx)) });
                for (call.args.items) |a| try compileExpr(self, a);
                _ = self.emit(OpCode.Call, BytecodeArg{ .Int = @as(i32, @intCast(1 + call.args.items.len)) });
            } else if (call.callee.* == .Identifier) {
                const name = call.callee.Identifier.name;
                const idx = self.addConst(ConstantValue{ .String = name });
                _ = self.emit(OpCode.LoadConst, BytecodeArg{ .Int = @as(i32, @intCast(idx)) });
                for (call.args.items) |a| try compileExpr(self, a);
                _ = self.emit(OpCode.Call, BytecodeArg{ .Int = @as(i32, @intCast(call.args.items.len)) });
                self.pushStackType(.Unknown);
            } else {
        ...
        .PropertyAccess => |prop| {
            try compileExpr(self, prop.target);
            _ = self.emit(OpCode.PropertyGet, BytecodeArg{ .String = prop.prop });
        },
        .ArrayLiteral => |arr| {
            for (arr.elements.items) |x| try compileExpr(self, x);
            _ = self.emit(OpCode.MakeArray, BytecodeArg{ .Int = @as(i32, @intCast(arr.elements.items.len)) });
        ...
        .MapLiteral => |map| {
            ...
            _ = self.emit(OpCode.MakeMap, BytecodeArg{ .Int = @as(i32, @intCast(map.pairs.items.len)) });
        ...
        .NewExpr => |new| {
            ...
            _ = self.emit(OpCode.Call, BytecodeArg{ .Int = @as(i32, @intCast(total_args)) });
            self.pushStackType(.Instance);
        },
        .MoveExpr => |me| {
            try compileExpr(self, me.expr);
            _ = self.emit(OpCode.OwnershipMove, .None);
        },
        .AddressOf => |addr| {
            try compileExpr(self, addr.expr);
            _ = self.emit(OpCode.BorrowCheck, .None);
            _ = self.emit(OpCode.AddressOf, .None);
            self.pushStackType(.Pointer);
        },
        .Deref => |d| {
            try compileExpr(self, d.expr);
            _ = self.emit(OpCode.AliveCheck, .None);
            _ = self.emit(OpCode.Deref, .None);
            self.pushStackType(.Instance);
        },
        .PointerMember => |pm| {
            try compileExpr(self, pm.expr);
            _ = self.emit(OpCode.AliveCheck, .None);
            _ = self.emit(OpCode.PropertyGet, BytecodeArg{ .String = pm.member });
        },
        // 语句级节点在表达式位置不应出现（解析器保证不可达）
        .StructDecl, .ClassDecl, .EnumDecl, .UnionDecl, .FuncDecl, .ImportStmt, .ExternDecl, .TypeExpr, .ExprStmt, .VarDecl, .Assign, .IfStmt, .WhileStmt, .ForStmt, .LoopStmt, .BreakStmt, .ContinueStmt, .ReturnStmt => {},
    ...
    if (std.mem.eql(u8, op, "=")) {
        switch (target.*) {
            .Identifier => |id| {
                ...
                const slot = self.allocateSlot(id.name);
                _ = self.emit(OpCode.StoreVar, BytecodeArg{ .Int = @as(i32, @intCast(slot)) });
            },
            .IndexAccess => |idx_acc| {
                try compileExpr(self, value);
                try compileExpr(self, idx_acc.target);
                try compileExpr(self, idx_acc.index);
                _ = self.emit(OpCode.IndexSet, .None);
            },
            .PropertyAccess => |prop| {
                try compileExpr(self, value);
                try compileExpr(self, prop.target);
                _ = self.emit(OpCode.PropertySet, BytecodeArg{ .String = prop.prop });
                _ = self.emit(OpCode.Pop, .None);
            },
            else => {},
        }
    } else {
        const bin_op: []const u8 = if (std.mem.eql(u8, op, "+=")) "+" else if (std.mem.eql(u8, op, "-=")) "-" else if (std.mem.eql(u8, op, "*=")) "*" else if (std.mem.eql(u8, op, "/=")) "/" else if (std.mem.eql(u8, op, "%=")) "%" else if (std.mem.eql(u8, op, "^=")) "^" else op;
        ...
        switch (target.*) {
            .Identifier => |id| {
                ...
                const oc: OpCode = if (self.binaryOpSpecialized(bin_op, var_type, value_type)) |oc_spec|
                    oc_spec
                else if (std.mem.eql(u8, bin_op, "+")) .BinaryAdd else if (std.mem.eql(u8, bin_op, "-")) .BinarySub else if (std.mem.eql(u8, bin_op, "*")) .BinaryMul else if (std.mem.eql(u8, bin_op, "/")) .BinaryDiv else if (std.mem.eql(u8, bin_op, "%")) .BinaryMod else if (std.mem.eql(u8, bin_op, "^")) .BinaryPow else return error.UnknownBinaryOp;
                _ = self.emit(oc, .None);
                const result_type: KnownType = blk: {
                ...
                _ = self.emit(OpCode.StoreVar, BytecodeArg{ .Int = @as(i32, @intCast(slot2)) });
            },
            .IndexAccess => |idx_acc| {
                try compileExpr(self, idx_acc.target);
                try compileExpr(self, idx_acc.index);
                _ = self.emit(OpCode.IndexGet, .None);
                try compileExpr(self, value);
                const oc: OpCode = if (std.mem.eql(u8, bin_op, "+")) .BinaryAdd else if (std.mem.eql(u8, bin_op, "-")) .BinarySub else if (std.mem.eql(u8, bin_op, "*")) .BinaryMul else if (std.mem.eql(u8, bin_op, "/")) .BinaryDiv else if (std.mem.eql(u8, bin_op, "%")) .BinaryMod else if (std.mem.eql(u8, bin_op, "^")) .BinaryPow else return error.UnknownBinaryOp;
                _ = self.emit(oc, .None);
                ...
                _ = self.emit(OpCode.StoreVar, BytecodeArg{ .Int = @as(i32, @intCast(tmp_slot)) });
                try compileExpr(self, idx_acc.target);
                try compileExpr(self, idx_acc.index);
                const tmp_slot2 = self.allocateSlot(tmp);
                _ = self.emit(OpCode.LoadVar, BytecodeArg{ .Int = @as(i32, @intCast(tmp_slot2)) });
                _ = self.emit(OpCode.IndexSet, .None);
            },
            .PropertyAccess => |prop| {
                try compileExpr(self, prop.target);
