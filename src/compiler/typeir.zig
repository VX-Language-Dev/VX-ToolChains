...
pub const TypeIRSimulator = struct {
    ...
    fn emit(self: *TypeIRSimulator, inst: TypedInstruction) void {
        self.body.append(self.allocator, inst) catch @panic("OOM");
    }

    fn pushVal(self: *TypeIRSimulator, var_id: VarId) void {
        self.stack.append(self.allocator, var_id) catch @panic("OOM");
    ...
    pub fn translateInst(self: *TypeIRSimulator, inst: *const Instruction, constants: []const ConstantValue) void {
        switch (inst.op) {
            ...
            .MakeArray => {
                ...
                var elems: std.ArrayList(VarId) = .empty;
                var i: usize = 0;
                while (i < count) : (i += 1) {
                    elems.append(self.allocator, self.popVal()) catch @panic("OOM");
                }
                const vid = self.allocVid();
                self.emit(TypedInstruction{ .MakeArray = .{ .base = 0, .args = elems.toOwnedSlice(self.allocator) catch @panic("OOM") } });
                self.pushVal(vid);
            },
            .IndexGet => {
            ...
            .Call => {
                ...
                while (i < num_args) : (i += 1) {
                    args.append(self.allocator, self.popVal()) catch @panic("OOM");
                }
                // 反转：栈顶是最后一个参数，恢复原始顺序
                ...
                if (callee_id) |fid| {
                    ...
                    self.emit(TypedInstruction{ .Call = .{
                        .func_id = fid,
                        .args = args.toOwnedSlice(self.allocator) catch @panic("OOM"),
                        .ext_name = null,
                    } });
                    self.pushVal(vid);
                } else {
                    ...
                    if (args.items.len > 0) {
                        ...
                        if (self.const_strings.get(first_vid)) |method_name| {
                            ...
                            var new_args: std.ArrayList(VarId) = .empty;
                            new_args.append(self.allocator, callee_vid) catch @panic("OOM");
                            var j: usize = 1;
                            while (j < args.items.len) : (j += 1) {
                                new_args.append(self.allocator, args.items[j]) catch @panic("OOM");
                            }
                            ext_name = self.allocator.dupe(u8, method_name) catch @panic("OOM");
                            real_args = new_args.toOwnedSlice(self.allocator) catch @panic("OOM");
                            ...
                            real_args = args.toOwnedSlice(self.allocator) catch @panic("OOM");
                        }
                    } else {
                        if (name_for_ext) |n| {
                            ext_name = if (self.extern_qualified_names.get(n)) |resolved|
