...
- **原生编译模式**（推荐）：通过 VXCO 中间文件生成独立的原生可执行文件，支持静态/动态链接
- **解释模式**：编译为字节码，由内置 VM 解释执行
...
- **完整编译流水线**：`.vx` 源码 → Token 流 → AST → 所有权检查 → 类型化 IR → VXCO 跨平台中间文件
- **原生编译支持**：默认静态链接生成独立可执行文件，自动检测外部依赖切换动态链接
- **VXCO 中间文件格式**：跨平台中间文件，不包含特定平台可执行特征，支持链接器自动适配目标平台
...
- **编译时宏系统**：支持参数化宏定义和展开，智能缓存机制，零运行时开销，详见 [宏系统文档](docs/macros.md)
...
└── src/
    ...
    ├── bytecode.rs          # VXOBJ v2/v3 + VXCO v1 序列化/反序列化（LZ4 压缩）
