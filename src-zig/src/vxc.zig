...
pub fn main(init: std.process.Init) !void {
    ...
    while (i < args.items.len) : (i += 1) {
        if (std.mem.eql(u8, args.items[i], "-o")) {
            ...
            opt_level = std.fmt.parseInt(u8, args.items[i], 10) catch 20;
    ...
    // 4) 编译
    var compiler = Compiler.init(allocator, VxSettings{ .stdlib_path = "" });
...
fn printUsage(prog: []const u8) void {
    ...
    std.debug.print("  --opt-level <n>   Optimization level (0-255)\n", .{});
