...
pub fn compile(self: *Compiler, ast: std.ArrayList(*const Stmt)) !CompiledModule {
    ...
    // 单态化（placeholder，直接返回原 AST）
    var stmts: std.ArrayList(*Expr) = .empty;
    defer stmts.deinit(self.allocator);
    for (ast.items) |s| {
        stmts.append(self.allocator, @constCast(s)) catch @panic("OOM");
    }
    const mono_ast = monomorphizeAst(stmts, self.allocator);
    defer mono_ast.deinit(self.allocator);

    var structs: std.ArrayList(struct { name: []const u8, fields: std.ArrayList([]const u8) }) = .empty;
    var classes: std.ArrayList(struct { name: []const u8, fields: std.ArrayList([]const u8) }) = .empty;
    ...
    for (mono_ast.items) |s| {
        switch (s.*) {
            ...
            .ClassDecl => |cd| {
                ...
                for (cd.methods.items) |m| {
                    if (m.* == .FuncDecl) {
                        ...
                        self.instructions = .empty;
                        const save_var_types = self.var_types.clone();
            ...
            .FuncDecl => |fd| {
                ...
                const save_var_types = self.var_types.clone();
                defer save_var_types.deinit();
                const save_var_slots = self.var_slots.clone();
...
fn monomorphizeAst(ast: std.ArrayList(*Expr), allocator: std.mem.Allocator) std.ArrayList(*Expr) {
    _ = allocator;
    _ = &ast;
    return ast;
}
