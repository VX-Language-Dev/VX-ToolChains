...
pub fn compileStmt(self: *Compiler, s: *const Stmt) !void {
    switch (s.*) {
        ...
        .VarDecl => |vd| {
            ...
            self.emit(OpCode.DefineVar, BytecodeArg{ .Int = @as(i32, @intCast(slot)) });
        },
        .IfStmt => |if_s| {
        ...
        .WhileStmt => |while_s| {
            ...
            self.emit(OpCode.Jump, BytecodeArg{ .Int = @as(i32, @intCast(start)) });
        ...
        .ForStmt => |for_s| {
            ...
            self.emit(OpCode.DefineVar, BytecodeArg{ .Int = @as(i32, @intCast(src_slot)) });
            const const_0 = self.addConst(ConstantValue{ .Int = 0 });
            self.emit(OpCode.LoadConst, BytecodeArg{ .Int = @as(i32, @intCast(const_0)) });
            const idx_slot = self.allocateSlot(idx_var);
            self.emit(OpCode.DefineVar, BytecodeArg{ .Int = @as(i32, @intCast(idx_slot)) });
            const start = self.instructions.items.len;
            const loop_info = LoopInfo{
                .start = start,
                .break_jumps = .empty,
            ...
            self.emit(OpCode.LoadVar, BytecodeArg{ .Int = @as(i32, @intCast(idx_slot2)) });
            const src_slot2 = self.allocateSlot(src_var);
            self.emit(OpCode.LoadVar, BytecodeArg{ .Int = @as(i32, @intCast(src_slot2)) });
            const const_len = self.addConst(ConstantValue{ .String = "len" });
            self.emit(OpCode.LoadConst, BytecodeArg{ .Int = @as(i32, @intCast(const_len)) });
            self.emit(OpCode.Call, BytecodeArg{ .Int = 1 });
            self.emit(OpCode.BinaryLt, .None);
            const exit_j = self.emit(OpCode.JumpIfFalse, .None);
            const src_slot3 = self.allocateSlot(src_var);
            self.emit(OpCode.LoadVar, BytecodeArg{ .Int = @as(i32, @intCast(src_slot3)) });
            const idx_slot3 = self.allocateSlot(idx_var);
            self.emit(OpCode.LoadVar, BytecodeArg{ .Int = @as(i32, @intCast(idx_slot3)) });
            self.emit(OpCode.IndexGet, .None);
            const var_slot = self.allocateSlot(for_s.var_name);
            self.emit(OpCode.DefineVar, BytecodeArg{ .Int = @as(i32, @intCast(var_slot)) });
            ...
            self.emit(OpCode.LoadVar, BytecodeArg{ .Int = @as(i32, @intCast(idx_slot4)) });
            const const_1 = self.addConst(ConstantValue{ .Int = 1 });
            self.emit(OpCode.LoadConst, BytecodeArg{ .Int = @as(i32, @intCast(const_1)) });
            self.emit(OpCode.BinaryAdd, .None);
            const idx_slot5 = self.allocateSlot(idx_var);
            self.emit(OpCode.StoreVar, BytecodeArg{ .Int = @as(i32, @intCast(idx_slot5)) });
            self.emit(OpCode.Jump, BytecodeArg{ .Int = @as(i32, @intCast(start)) });
            const exit_pc = self.instructions.items.len;
            self.patch(exit_j, exit_pc);
            const last_info = &self.loop_stack.items[self.loop_stack.items.len - 1];
        ...
        .MatchStmt => |match_s| {
            ...
            self.emit(OpCode.StoreVar, BytecodeArg{ .Int = @as(i32, @intCast(subject_slot)) });
            ...
            for (match_s.arms.items, 0..) |*arm, arm_idx| {
                ...
                self.emit(OpCode.LoadVar, BytecodeArg{ .Int = @as(i32, @intCast(subject_slot)) });
                self.emit(OpCode.BinaryEq, .None);
