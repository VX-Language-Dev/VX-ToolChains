...
pub fn main(init: std.process.Init) !void {
    ...
    // 写入输出文件
    const out_path = try blk: {
        if (output) |o| break :blk o;
        if (std.mem.lastIndexOfScalar(u8, input, '.')) |dot| {
            break :blk try std.fmt.allocPrint(allocator, "{s}.decompiled.vx", .{input[0..dot]});
        } else {
            break :blk try std.fmt.allocPrint(allocator, "{s}.decompiled.vx", .{input});
        }
    };
